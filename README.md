...
Project Overview
This project implements a Deep Learning pipeline to enhance the spatial resolution of Synthetic Aperture Radar (SAR) imagery. SAR data is notoriously difficult to process due to speckle noise and complex geometry. This project utilizes a Very Deep Super Resolution (VDSR) inspired architecture with custom Residual Blocks to reconstruct high-frequency spatial details from low-resolution inputs.
The data is sourced from the Capella Space Open Data catalog (IEEE Data Contest collection) and retrieved dynamically via the STAC API.
...
PSNR:      ~19.50 dB                9.89 dB                20.09 dB
...
How to Run
Open SAR_Super_Resolution.ipynb in Google Colab.

Ensure GPU acceleration is enabled (Runtime -> Change runtime type -> T4 GPU).


Data Source
This project utilizes the (Capella Space Open Data Catalog).
Collection URL: https://capella-open-data.s3.us-west-2.amazonaws.com/stac/capella-open-data-ieee-data-contest/collection.json](https://capella-open-data.s3.us-west-2.amazonaws.com/stac/capella-open-data-ieee-data-contest/collection.json)
