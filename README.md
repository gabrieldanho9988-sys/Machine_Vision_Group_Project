...
1. **Detect** — segment chips from the green conveyor background
2. **Verify** — measure dimensions against reference; flag anomalies as `FAKE`
3. **Classify** — determine color tier using glare-resistant HSV thresholds
4. **Predict** — crop and orient the chip, pass to the CNN (`.h5` model) for value prediction
5. **Sum** — accumulate values of authenticated chips and render the total on the live feed
